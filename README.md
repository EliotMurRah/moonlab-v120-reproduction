...
| Claim | Verdict |
|---|---|
| C1 — batched sampling beats Stim | **Partial**: speed direction reproduces on every row (up to 11.8× MT), but the shipped cross-engine correctness gate fails deterministically on this host (open finding F2 below) |
...
| F2 — batch-sampling correctness gate fails deterministically | **Open** — 6.01σ ST / 3.59σ MT detector-marginals gate on `surface_code_d5_r8_noisy`; fire rates ml=0.01350 vs stim=0.01347; bit-identical across reruns (front internally seeded) |
